...
export function DiscussionPageClient({ latestComments }: { latestComments: Comment[] }) {
  const { t } = useLanguage()
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-4xl mx-auto">
        ...
        <p className="text-lg text-foreground/80 mb-8">{t("discussion.description")}</p>
        ...
        {discussionGroups.length === 0 ? (
          <div className="bg-surface-2 rounded-xl p-8 text-center">
            ...
            <h2 className="text-xl font-medium mb-2">{t("discussion.empty")}</h2>
            <p className="text-muted-foreground mb-6">{t("discussion.empty.description")}</p>
            <Button asChild>
          ...
          <div className="space-y-8">
            {discussionGroups.map((group) => (
              <div key={group.caveId} className="bg-surface-2 rounded-xl overflow-hidden shadow-sm">
                <div className="bg-primary/10 p-4 flex items-center justify-between">
                  <h2 className="text-xl font-semibold text-primary">
                    <Link href={`/caves/${group.caveId}`} className="hover:underline">
                      {group.caveName}
                    </Link>
                  </h2>
                  <span className="text-sm text-muted-foreground">
                    {group.commentCount} {group.commentCount === 1 ? t("discussion.comment") : t("discussion.comments")}
                  </span>
                </div>
                    ...
                    <div key={comment.id} className="p-4">
                      <div className="flex items-start">
                        <Avatar className="h-8 w-8 mr-3">
                          ...
                          <AvatarFallback>{comment.user_name ? getInitials(comment.user_name) : "U"}</AvatarFallback>
                        </Avatar>
                ...
                <div className="bg-surface-3 p-3 flex justify-end">
                  <Button asChild variant="ghost" size="sm" className="text-accent hover:text-accent/90">
                    <Link href={`/caves/${group.caveId}`} className="flex items-center gap-1">
                      {t("discussion.view")}
