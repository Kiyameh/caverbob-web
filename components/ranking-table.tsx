...
import {
  ...
  terrainTypes,
  type TerrainType,
} from '@/components/terrain-filter'
...
export default function RankingTable({type}: RankingTableProps) {
  ...
  const filteredData = rankingData.filter((cave) => {
    ...
    if (type === 'longest' || type === 'deepest') {
      ...
      const matchesTerrain = selectedTerrains.includes(
        caveData.terrainType as TerrainType
      )
      return matchesSearch && matchesTerrain
    } else if (type === 'pits') {
      const pitData = cave as (typeof mockDeepPitsData)[0]
      return (
        pitData.cave.toLowerCase().includes(searchLower) ||
        pitData.country.toLowerCase().includes(searchLower)
      )
    } else if (type === 'chambers') {
      const chamberData = cave as (typeof mockLargestChambersData)[0]
      return (
        chamberData.cave.toLowerCase().includes(searchLower) ||
        chamberData.country.toLowerCase().includes(searchLower)
      )
    }
                  ...
                  return (
                    <TableRow key={caveData.id}>
                      ...
                      <TableCell className="text-right">
                        {formatLength(caveData.length)}
                      </TableCell>
                      <TableCell className="text-right">
                        {formatDepth(caveData.depth)}
                      </TableCell>
                      <TableCell className="text-right">
                        {caveData.entrances}
                      </TableCell>
          ...
          <Table>
            ...
            <TableBody>
              {sortedData.length > 0 ? (
                sortedData.map((cave) => {
                  const chamberData =
                    cave as (typeof mockLargestChambersData)[0]
                  return (
                    <TableRow key={chamberData.id}>
                      ...
                      <TableCell className="text-right">
                        {formatArea(chamberData.area)}
                      </TableCell>
                      <TableCell className="text-right">
                        {formatVolume(chamberData.volume)}
                      </TableCell>
                      <TableCell>{chamberData.source}</TableCell>
                      <TableCell>
                        {formatDate(chamberData.lastUpdated)}
                      </TableCell>
                    </TableRow>
                  )
                })
